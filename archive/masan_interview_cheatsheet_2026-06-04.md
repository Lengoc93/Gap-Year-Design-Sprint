...
At Trusting Social, I optimized right-person-contact verification for the AI Voice Agent — the step where the agent confirms it's speaking to the actual debtor before proceeding to negotiation.

**The data insight:** I ran a taxonomy analysis across historical conversations, labeling call intent by customer phone number × debt ID. The finding: 40% of inbound calls had classifiable intent — the debtor was calling back with a specific purpose (make a payment, dispute an amount, request a plan). But the verification flow was treating all calls identically — same cold-open script whether the person called us or we called them. An inbound caller who already wanted to pay was being forced through the same "are you willing to engage?" evaluation as a cold outbound call. That's friction that kills the exact behavior you want.

**The redesign:** I made the verification flow context-aware. For inbound calls with classifiable intent: classify the call intent → identify the "willing to pay" signal → skip the willing-to-pay evaluation entirely and route directly to setting up a promise-to-pay. The agent recognized WHERE the debtor already was in their decision journey and removed the steps that didn't belong.

**The rollout decision:** Direct rollout, not A/B test. The estimated impact on in-call payment conversion was so much larger than the downside risk of false-negative or false-positive intent classification that testing would have been a cost, not a safeguard.

**Result:** +9 percentage points on right-person-contact verification success rate — which directly lifted downstream promise-to-pay conversion.

**The principle for Masan:** The same logic applies to any data product serving operational users. A BU supervisor pulling up a dashboard at 7am to check overnight shift output doesn't need the same onboarding flow as a first-time user. A store manager checking inventory doesn't need the same navigation as a regional director reviewing cross-store KPIs. The data tells you who the user is and what they're here for — the product's job is to recognize that context and remove the friction that doesn't belong. That's what "data-driven" actually means in an operational environment: not more dashboards, but smarter product behavior based on user context.

> **Tactical note (updated Jun 9 — JD reframe):** This answer lands better for the actual Data PM role than a consumer fintech example. It demonstrates: (1) using data taxonomy to identify a behavioral pattern, (2) redesigning an operational flow to be context-aware, (3) making a confident rollout decision based on impact estimation, (4) measurable outcome. The Masan bridge now targets operational data products (the JD scope) rather than WIN Membership. If anh Son probes the taxonomy method: "I ran intent classification across the historical conversation corpus, segmented by customer phone number × debt ID, and built a label taxonomy of inbound call purposes. The 40% was the proportion with high-confidence classifiable intent — the rest were ambiguous or misrouted."
